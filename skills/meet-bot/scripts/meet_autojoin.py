    ...
    try:
        ...
        for row in c.execute("""
            ...
            AND name IN ('SID','HSID','SSID','APISID','SAPISID','__Secure-1PSID','__Secure-3PSID',
                         '__Secure-1PSIDTS','__Secure-3PSIDTS','OTZ','NID','__Secure-ENID')
        """):
...
def join_via_chrome_cdp():
    ...
    with sync_playwright() as p:
        try:
            ...
            log(f"Chrome CDP baglanti hatasi: {e}")
            return False
        ...
        time.sleep(5)
        if "can't join" in page.evaluate("document.body.innerText").lower():
            log("Toplantiya erisilemiyor")
            page.close(); return False
        ...
        time.sleep(3)
        if "can't join" in page.evaluate("document.body.innerText").lower():
            log("Companion mode basarisiz"); page.close(); return False
        log("✅ TOPLANTIDA!")
        # Turn on captions
        page.evaluate("""() => {
            for (const b of document.querySelectorAll('button')) {
                ...
                if ((aria.includes('altyaz') && aria.includes('aç')) or 
                    ...
                    (text.includes('closed_caption_off'))) { b.click(); return; }
            }
        }""")
        log("Captions toggled"); time.sleep(2)
        ...
        log("Language Turkish"); time.sleep(3)
        log("Starting caption poll...")
        seen = set(); last_log = time.time()
        try:
            while True:
                caps = page.evaluate("""() => {
                    const found = [];
                ...
                if isinstance(caps, list):
                    for c in caps:
                        if c not in seen:
                            seen.add(c); ts = time.strftime('%H:%M:%S')
                            with open(transcript_path, "a") as f: f.write(f"[{ts}] {c}\n")
                            print(f"[CAPTION] {c}", flush=True)
                now = time.time()
                if now - last_log >= 60:
                    log(f"Alive: {len(seen)} lines"); last_log = now
                time.sleep(3)
        except KeyboardInterrupt: log("Stopped")
        except Exception as e: log(f"Error: {repr(e)}")
...
if __name__ == "__main__":
    ...
    if cookie_ok: log("Cookies exported. Falling through to Method 2: Chrome CDP...")
    success = join_via_chrome_cdp()
    if not success:
        log("All methods failed.")
        print("\nNe yapmali:")
        print("  1. Edel'e bildir: Toplantiya otomatik katilamadim")
