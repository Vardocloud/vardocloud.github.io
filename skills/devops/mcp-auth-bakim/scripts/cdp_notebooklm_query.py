#!/usr/bin/env python3
"""CDP NotebookLM Sorgu v5 — Kanıtlanmış (5 Tem 2026).
Kullanım: python3 cdp_notebooklm_query.py "soru" [notebook_id]
Notebook ID'leri: bdt, apa, veya doğrudan UUID
"""
import websocket, json, time, urllib.request, sys

NOTEBOOKS = {
    "bdt": "a4fe729d-c561-4238-9bea-81bea8e3dcbc",
    "apa": "c44469fe-a69a-4a86-8dd8-756c2f365109",
}
...
def ask(question, notebook_id="bdt"):
    ...
    # loadEventFired bekle (SPA render için kritik)
    ws.settimeout(60)
    while True:
        try:
            d = json.loads(ws.recv())
            if d.get('method') == 'Page.loadEventFired':
                time.sleep(5)  # SPA component mount süresi
                break
        except:
            ws.close()
            return None

    # Önceki mesaj sayısını al (cevap tespiti için)
    prior = eval_js(ws, 'document.querySelectorAll(".message-content").length')
    if prior is None:
        prior = 0

    # Textarea'ya odaklan
    eval_js(ws, 'document.querySelector("textarea")?.focus()')
    time.sleep(0.3)

    # Soruyu karakter-karakter yaz (human-like)
    for ch in question:
    ...
    # Enter sequence (rawKeyDown + char + keyUp)
    ...
    for i in range(40):
        ...
        (function(){
            ...
            var c = msgs.length;
            var last = c > 0 ? msgs[c-1].innerText?.substring(0,2000)||'' : '';
            return JSON.stringify({count:c, last:last});
        })()
        """)
        if current:
