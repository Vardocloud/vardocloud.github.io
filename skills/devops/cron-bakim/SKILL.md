...
### 4b. Redundant Job Tespiti ve Temizliği

**Uyarı işaretleri:**
- Bir job'ın yaptığı işi başka bir job/system daha iyi yapıyor
- Job başka bir sistem tarafından replace edilmiş ama eski job silinmemiş (örn: `nlm-auth-refresh.sh` → `nb_keepalive.py`)
- Script hatası alan job'ın yerini alan yeni bir sistem var ama eski job paused bekliyor
- Dokümantasyonda hala eski job'a referans var ama gerçekte kullanılmıyor

**Ne yapılmalı:**
1. Eski job'ın replace edildiğini doğrula — yeni sistem çalışıyor mu kontrol et (`cronjob list` ile son durum)
2. Edel'e danışmadan eminsen **direkt sil** (`cronjob action=remove`). Job zaten paused ise ve replace edilmişse, pause'ta beklemesinin anlamı yok.
3. Emin değilsen Edel'e sor: "Eski job'ı sileyim mi, yoksa yedek olarak kalsın mı?"
4. **Dokümantasyon temizliği:** Eski job'a referans veren skill veya referans dosyaları varsa, onları da güncelle (varsa `warp-proxy` skill'inde olduğu gibi eski `refresh_cookies.py` referansı).
5. Silinen job'ın script dosyası hala diskte duruyorsa, tamamen ortadan kalktığından emin olmak için sor: "Script dosyasını da silelim mi?"

### 4c. Cron Job Kaynak Tüketimi Tahmini

Edel "Bu job ne kadar kaynak tüketiyor?" dediğinde veya bir job'ı ücretli provider'dan ücretsize taşımayı düşünürken, aşağıdakileri değerlendir:

**Prompt-based (LLM-driven) job için:**

| Faktör | Nasıl Ölçülür | Anlamı |
|--------|--------------|--------|
| **Job tipi** | `no_agent: true` mi, `false` mu? | no_agent sadece script çalıştırır, neredeyse sıfır LLM maliyeti. `false` ise her çalışmada tam agent döngüsü → **pahalı** |
| **Skills sayısı** | `skills` alanındaki skill adedi | Her skill yüklendiğinde system prompt'a eklenir. 2 skill = 1 skill'den ~2x input token. sohbet + llm-wiki gibi büyük skill'ler daha da ağırdır. |
| **Prompt uzunluğu** | `prompt` alanının karakter sayısı | Prompt'un kendisi her çalışmada input token'ı olarak tüketilir. 2500+ karakterlik promptlar ciddi maliyettir. |
| **Tool call sayısı** | Prompt'ta geçen web_search, web_extract, write_file sayısı | Her tool call = yeni bir tur (input + output). 6 web_search = 6 tur, bu da toplam token'ı katlar. |
| **Çalışma sıklığı** | `schedule` alanı | Günde 1 kere vs her saat başı. Sıklık arttıkça maliyet lineer artar. |
| **Completed runs** | `repeat.completed` alanı | Kaç kez çalıştığını gösterir. 66 run × her run için ~100K token = 6.6M token tüketim. |
| **Duration** | `last_run_at` ile bir önceki çalışma arasındaki fark | 14 dk süren bir job, uzun bir agent döngüsü demektir (çok tur, çok output). |
| **Model/provider** | `model` + `provider` alanları | GLM-5.1 (büyük, pahalı) vs deepseek-v4-flash-free (küçük, ücretsiz). Aynı işi yapan job'lar arasında karşılaştır. |
| **Output miktarı** | Job'ın ürettiği çıktının boyutu | 8-10 haber = orta. Uzun analiz = yüksek. |

**Tahmin formülü (kabaca):**
```
Her çalışma için:
  input_token ≈ system_prompt + skill_1 + skill_2 + ... + prompt + tool_calls(web_result_1 + ...)
  output_token ≈ final_response
  Her tool_call = 1 tur daha (input+output)
  
Aylık maliyet = (input_token + output_token) × runs_per_month × price_per_token
```

**Pratik örnek — Bundle Gündem İşleme (93582f1545d2):**
- Job tipi: prompt-based (no_agent=false) → **pahalı**
- Skills: sohbet (büyük) + llm-wiki (büyük) → **çok input token**
- Prompt: ~2500 karakter → orta
- Tool calls: 6+ web_search + dedup + wiki write → **çok tur**
- Schedule: günde 1 kere → düşük sıklık (avantaj)
- Completed: 66 runs → **66 kez GLM-5.1 tüketmiş**
- Duration: ~14 dk → uzun agent döngüsü
- Model: glm-5.1 / opencode-go → **ücretli, büyük model**
- **Kanaat:** Haber toplama + 5N1K formatlama gibi hafif bir iş için GLM-5.1 çok ağır. deepseek-v4-flash-free fazlasıyla yeterli. Günde 1 kere çalıştığı için rate limit sorunu olmaz.
- **Aksiyon:** Modeli `deepseek-v4-flash-free` / `opencode-zen`'e taşı. 

**Ne zaman migrate et:**
- Job hafif bir iş yapıyorsa (formatlama, özet, haber toplama) ama ağır bir model kullanıyorsa
- Job günde 1-2 kere çalışıyorsa (free tier rate limiti aşmaz)
- Aynı provider'da benzer job'lar başarıyla çalışıyorsa
- Job'ın kalitesi daha küçük modelle korunabiliyorsa (Türkçe haber formatlama için GLM-5.1 gerekmez)
...
- **Cron raporu/monitör çıktısını pasif okuma — yapma!** Edel bir cron raporu gönderdiğinde sadece "hataları tespit ettim, şunlar var" demek yetmez. Raporun içindeki **atlanmış fırsatları da yakala:**
  - Redundant job var mı? (bir sistem diğerini replace etmiş ama eskisi hala duruyor)
  - Paused job'ların neden paused olduğu biliniyor mu? Yoksa "belki unutulmuş" mu?
  - "İncelenmeli / gerektiğinde bakılır" gibi pasif notlar var mı? Bunlar aksiyona dönüştürülmeli.
  - WARP down gibi uzun süredir devam eden sorunlar var mı? Bunu her raporda tekrar not etme — ya çöz ya da kabul et.
  - Job'ların model/provider maliyeti optimal mi? glm-5.1 gibi pahalı bir model basit bir formatlama işi için kullanılıyor olabilir.
  
  **Altın kural:** Bir raporu incelerken sadece raporun söylediklerini değil, **söylemediği şeyleri de** ara. Raporda WARP için "incelenmeli" yazıyorsa ve bu 3. raporda aynı not varsa, bu bir aksiyon eksikliğidir — çözüm üret.
