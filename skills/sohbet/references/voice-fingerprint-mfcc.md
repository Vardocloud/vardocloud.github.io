...
## Integration Points in vanitas_ses.py

1. **Module level:** `_load_voiceprint()`, `verify_voiceprint()`, `_extract_mfcc_mean()`
2. **WebSocket scope:** `utterance_audio = bytearray()`, `last_utterance_is_edel = True`
3. **Main loop (line ~872):** `utterance_audio.extend(raw)` before Soniox forward
4. **flush_utterances:** verify → set `last_utterance_is_edel` → clear buffer
5. **Speaker change flush (process_soniox):** same verify + clear pattern
6. **_handle_reply:** voiceprint priority over Soniox heuristic fallback

## Priority Logic

```
if voiceprint enrolled AND result != None:
    use voiceprint result
else:
    fallback: Soniox speaker tag heuristic ("first speaker = Edel")
```
