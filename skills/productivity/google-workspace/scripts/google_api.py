...
Usage:
  ...
  python google_api.py calendar create --summary "Meeting" --start DATETIME --end DATETIME [--reminder 120]
...
_env_token = os.environ.get("HERMES_GOOGLE_TOKEN_PATH")
TOKEN_PATH = Path(_env_token) if _env_token else HERMES_HOME / "google_token.json"
...
def _stored_token_scopes() -> list[str]:
    try:
        ...
        return list(SCOPES)
    # Handle both "scopes" (list) and "scope" (str/space-separated) keys
...
def calendar_create(args):
    ...
    if args.reminder and args.reminder > 0:
        event["reminders"] = {
            "useDefault": False,
            "overrides": [
...
def main():
    ...
    p.add_argument("--reminder", type=int, default=0, help="Reminder minutes before event (0=no reminder). Typical: 120 for 2h")
    p.add_argument("--calendar", default="primary")
