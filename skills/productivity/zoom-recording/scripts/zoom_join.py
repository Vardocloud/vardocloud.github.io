...
def check_join_success(ws):
    ...
    try:
        d = json.loads(r)
        if d.get('isWaiting'): return False, 'bekleme odasinda'
        if d.get('hasUnmute') or d.get('hasLeave'): return True, f"iceri: {d.get('bodyStart','')[:60]}"
...
def join_zoom(meeting_id, passcode, display_name, zoom_url=None):
    ...
    if not tab: return False, "Zoom tab bulunamadi"
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Zoom toplantisina katil')
    parser.add_argument('--meeting', required=True, help='Meeting ID')
    parser.add_argument('--pwd', default='', help='Passcode')
    parser.add_argument('--name', default='Sudenaz', help='Gorunen isim')
